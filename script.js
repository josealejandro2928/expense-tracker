...
function updateDom() {
    ...
    <p class="green" style="margin-top: 0px; font-size: 18px;">${income.toFixed(2)} CUP</p>`
    expenseElm.innerHTML = `<p style="text-transform: uppercase;">Gasto</p>
    <p class="red" style="margin-top: 0px;font-size: 18px;">${expense.toFixed(2)} CUP</p>`
    let historiHtml = `<h3> Histórico</h3>`;
    historiHtml += allTransaction.map(x => {
        return ` <div class="history-el ${(x.type == 'income'?'green':'red')}">
                    <div class="removeBtn">
                        x
                    </div>
                    <span style="flex: 1 1 100%">${x.text}</span>
                    <span style="flex: 1 1 140px;text-align: right;">${x.amount} CUP</span>
                </div>`
    }).join('');
...
addBtn.addEventListener('click', () => {
    ...
    if (!invalid) {
        ...
        console.log("🚀 ~ file: script.js ~ line 71 ~ addBtn.addEventListener ~ allTransaction", allTransaction)
        updateDom();
    }
...
history.addEventListener('click', (el) => {
    ...
    if (element.classList.contains('removeBtn')) {
        ...
        setTimeout(() => {
            let index = [...document.querySelectorAll('.history-el')].indexOf(historiEl);
